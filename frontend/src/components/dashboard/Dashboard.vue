    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="bg-white rounded-lg border">
        <div class="px-5 py-3 border-b flex justify-between items-center">
          ...
          <router-link to="/companies" class="text-sm text-blue-600 hover:underline">View all</router-link>
        </div>
        <div v-if="companyStore.loading" class="p-5"><Spinner /></div>
        <div v-else-if="companyStore.error" class="p-5 text-sm text-red-600">{{ companyStore.error }}</div>
        <div v-else>
          <div v-for="c in companyStore.companies" :key="c.id"
      ...
      <div class="bg-white rounded-lg border">
        ...
        <div v-if="vacancyStore.events.length" class="divide-y max-h-80 overflow-y-auto">
          <div v-for="(evt, i) in vacancyStore.events" :key="i" class="px-5 py-2.5 text-sm">
            ...
            <span class="text-gray-700">{{ evt.vacancy?.translations?.[0]?.title || 'Unknown title' }}</span>
            <span class="text-gray-400 ml-2 text-xs">{{ formatTime(evt.timestamp) }}</span>
          </div>
        </div>
        <div v-else class="p-5 text-sm text-gray-400 text-center">
          Waiting for updates...
        </div>
      </div>
    </div>
...
function clearEvents() { vacancyStore.events.value = []; }
